...
{if $isX}* un groupe promo, qui te permet de garder le contact avec tes cocons : tu {if $promoGroup}es{else}n’es pas{/if} inscrit à ton groupe promo sur Polytechnique.net{if !$promoGroup}, n'hésite pas à t'inscrire [[http://www.polytechnique.net/{$promoX}|ici]] !{elseif $mlpromo} ainsi qu'à la liste promo@{$promoX}.polytechnique.org.{else} mais pas à la liste promo@{$promoX}.polytechnique.org, n'hésite pas à t'inscrire [[http://listes.polytechnique.org/members/promo_{$promoX}.polytechnique.org|ici]].{/if}{/if}
